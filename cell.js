...
(function ($root) {
    ...
    var Membrane = {
        ...
        inject: function ($host, gene, namespace, replace) {
            ...
            if (replace && $host) {
                ...
                $node = $replacement;
                    ...
                    gene.$type && (gene.$type === "head" || gene.$type === "body") &&
                    $root.document.getElementsByTagName(gene.$type)
                    ) {
        ...
        add: function ($parent, gene, index, namespace) {
            ...
            if (
                    index !== null && index !== undefined && $parent.childNodes &&
    ...
    var Phenotype = {
            ...
            if (key[0] === "$") {
                if (key === "$type") {
                    ...
                    if (val.toLowerCase() !== tag) {
                        ...
                        var replacement = fragment.$build(
                                $node.Genotype,
                                $node.Inheritance,
                                null,
                                $node.Meta.namespace
        ...
        $type: function (model, namespace) {
            ...
            if (model.$type === "svg") {
                ...
                if (model.$text && typeof model.$text === "function") {
                    model.$text = Phenotype.multiline(model.$text);
                }
        ...
        $init: function ($node) {
            Nucleus.tick.call($root, function () {
                if ($node.Genotype && $node.Genotype.$init) {
                    Nucleus.bind($node, $node.Genotype.$init)();
                }
            });
        },
    ...
    var God = {
        detect: function ($context) {
            ...
            return Object.keys($context).filter(function (k) {
                if (k === "onmozfullscreenerror" || k === "onmozfullscreenchange") {
                    return false; // deprecated by Mozilla, throw deprecation message in browser console
                }

                try {
                    if (
                            /webkitStorageInfo|webkitIndexedDB/.test(k) ||
                            $context[k] instanceof $root.Element
                            ) {
                        return false; // Only look for plain javascript object
                    }
                    return $context[k] &&
                            Object.prototype.hasOwnProperty.call($context[k], "$cell");
                } catch (e) {
                    return false;
                }
            }).map(function (k) {
        ...
        plan: function ($context) {
            ...
            if ($root.NodeList && !$root.NodeList.prototype.forEach) {
                $root.NodeList.prototype.forEach = Array.prototype.forEach; // NodeList.forEach override polyfill
            }
        },
    ...
    window.celljs = true; // is cell.js added
}(this));
